...
public class EntityAlpaca extends AbstractChestHorse implements IRangedAttackMob
{
    ...
    @Nullable
    private EntityAlpaca caravanHead;
    @Nullable
    private EntityAlpaca caravanTail;
    ...
    protected void initEntityAI()
    {
        ...
        this.tasks.addTask(9, new EntityAIAvoidEntity(this, EntityPuma.class, 10.0F, 2.2D, 2.2D));
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata)
    {
        ...
        if (livingdata instanceof EntityAlpaca.GroupData)
        {
            i = ((EntityAlpaca.GroupData)livingdata).variant;
        }
        else
        {
            i = this.rand.nextInt(4);
            livingdata = new EntityAlpaca.GroupData(i);
        }
    ...
    {
        return otherAnimal != this && otherAnimal instanceof EntityAlpaca && this.canMate() && ((EntityAlpaca)otherAnimal).canMate();
    }

    public EntityAlpaca createChild(EntityAgeable ageable)
    {
        EntityAlpaca entityllama = new EntityAlpaca(this.world);
        this.setOffspringAttributes(ageable, entityllama);
        EntityAlpaca entityllama1 = (EntityAlpaca)ageable;
    ...
    public void joinCaravan(EntityAlpaca caravanHeadIn)
    {
    ...
    @Nullable
    public EntityAlpaca getCaravanHead()
    {
    ...
    static class AIDefendTarget extends EntityAINearestAttackableTarget<EntityWolf>
        {
            public AIDefendTarget(EntityAlpaca llama)
            {
    ...
    static class AIHurtByTarget extends EntityAIHurtByTarget
        {
            public AIHurtByTarget(EntityAlpaca llama)
            {
                super(llama, false);
            }
            ...
            {
                if (this.taskOwner instanceof EntityAlpaca)
                {
                    EntityAlpaca entityllama = (EntityAlpaca)this.taskOwner;
